...
def match_mentioned_concepts(sents, answers):
    ...
    for sid, s in tqdm(enumerate(sents), total=len(sents)):
        ...
        answer_concepts = [a for a in answer if (a in cpnet_vocab) and (a not in blacklist) and (a in model_vocab)]
        
        res.append({"sent": s, "qc": list(question_concepts), "ac": list(answer_concepts)})
    return res
...
def read_jsonl(data_path):
    ...
    concept = []
    for d in data:
        source.append(' '.join(d["source"]))
        concept.append(d["concept"])
    print(source[:3])
    print(concept[:3])
...
SRC_FILE = DATA_PATH + "/{}/source.jsonl"

read_model_vocab(config["paths"]["gpt2_vocab"])

TYPE='train'
src, cpt = read_jsonl(SRC_FILE.format(TYPE))
grounding_sentences(src, cpt, TYPE, DATA_PATH)
