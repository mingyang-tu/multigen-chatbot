...
def process(input_path, output_path, T, max_B):
    ...
    for ex in tqdm(data):
        ...
        e, found, avg_nodes = find_neighbours_frequency(source, target, T, max_B)
        avg_len += avg_nodes
        examples.append(e)
...
def load_target_concepts(data_path):    
    global concept2id, target_concepts_id, config
    target_concepts = []
    target_concepts_id = []

    path = data_path + "/train/concepts_nv.json"
    with open(path, 'r') as f:
        for line in f.readlines():
            line = json.loads(line)
            target_concepts.extend(line['ac'])

        target_concepts = list(set(target_concepts))

    
    filtered_target_concepts = []
    for x in target_concepts:
        if concept2id.get(x, False):
            target_concepts_id.append(concept2id[x])
            filtered_target_concepts.append(x)

    with open(data_path + "/target_concepts.txt", 'w') as f:
        for line in filtered_target_concepts:
            f.write(str(line) + '\n')
...
load_target_concepts(DATA_PATH)
